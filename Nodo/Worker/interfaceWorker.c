...
char* serializar_item_prog_reduccion_global(item_programa_reduccion_global* item_programa_reduccion_local){
	uint32_t total_size = getLong_one_item_reduccion_global(item_programa_reduccion_local);
	char *serializedPackage = malloc(sizeof(char)*total_size);

	int offset = 0;

	serializarDato(serializedPackage,&(item_programa_reduccion_local->ip_worker),sizeof(char[LENGTH_IP]),&offset);
	serializarDato(serializedPackage,&(item_programa_reduccion_local->puerto_worker),sizeof(uint32_t),&offset);
	serializarDato(serializedPackage,&(item_programa_reduccion_local->archivo_temp_red_local),sizeof(char[LENGTH_RUTA_ARCHIVO_TEMP]),&offset);
...
char* serializar_items_prog_reduccion_global(item_programa_reduccion_global** items_programa_reduccion_global, uint32_t cantidad_item){
	item_programa_reduccion_global* aux_items_programa_reduccion_global = *items_programa_reduccion_global;
	uint32_t total_size = getLong_items_reduccion_global(aux_items_programa_reduccion_global, cantidad_item);
	char *serializedPackage = malloc(sizeof(char)*total_size);
	...
	for (i = 0; i < cantidad_item; i++) {
		char* serialized_item = serializar_item_prog_reduccion_global(&aux_items_programa_reduccion_global[i]);//TODO: ver como pasarle el puntero como parametro
		uint32_t size_item = getLong_one_item_reduccion_global(&aux_items_programa_reduccion_global[i]);
...
char* serializarSolicitudProgramaReduccionGlobal(solicitud_programa_reduccion_global* solicitud){
	...
	char* serialized_items = serializar_items_prog_reduccion_global(&(solicitud->items_programa_reduccion_global),solicitud->cantidad_item_programa_reduccion);
	serializarDato(serializedPackage,serialized_items,sizeof(char)*size_items,&offset);
	free(serialized_items);
...
item_programa_reduccion_global* deserializar_items_programa_reduccion_global(char* serialized, uint32_t items_cantidad){
	...
	item_programa_reduccion_global* workers = malloc(sizeof(item_programa_reduccion_global)*items_cantidad);
	int i;
	for (i = 0; i < items_cantidad; i++) {
		uint32_t size_item;
		deserializarDato(&(size_item),serialized,sizeof(uint32_t),&offset);
		char* serialized_item = malloc(sizeof(char)*size_item);
		deserializarDato(serialized_item,serialized,size_item,&offset);
		item_programa_reduccion_global* aux = deserializar_item_prog_reduccion_global(serialized_item);
		workers[i] = *(aux);
		free(aux);
		free(serialized_item);
	}
