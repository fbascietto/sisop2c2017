...
uint32_t getLong_one_item_prog_reduccion_global(t_worker* items_programa_reduccion_global){
	uint32_t longitud = 0;
...
uint32_t getLong_items_prog_reduccion_global(t_worker* items_programa_reduccion_global, uint32_t cantidad){
	uint32_t total = 0;
	int i;
	for(i=0; i<cantidad; i++){
		total += sizeof(uint32_t);
		total += getLong_one_item_prog_reduccion_global(&(items_programa_reduccion_global[i]));
	}
	return total;
}
...
uint32_t getLong_SolicitudProgramaReduccionGlobal(solicitud_programa_reduccion_global* solicitud){
	...
	uint32_t size_items = getLong_items_prog_reduccion_global(solicitud->workers,solicitud->cantidad_item_programa_reduccion);
...
char* serializar_item_prog_reduccion_global(t_worker* item_reduccion_global){
	uint32_t total_size = getLong_one_item_prog_reduccion_global(item_reduccion_global);
...
char* serializar_items_prog_reduccion_global(t_worker** items_reduccion_global, uint32_t cantidad_item){
	t_worker* aux_items_reduccion_global = *items_reduccion_global;
	uint32_t total_size = getLong_items_prog_reduccion_global(aux_items_reduccion_global, cantidad_item);
	char *serializedPackage = malloc(sizeof(char)*total_size);
	...
	for (i = 0; i < cantidad_item; i++) {
		char* serialized_item = serializar_item_prog_reduccion_global(&aux_items_reduccion_global[i]);//TODO: ver como pasarle el puntero como parametro
		uint32_t size_item = getLong_one_item_prog_reduccion_global(&aux_items_reduccion_global[i]);
...
char* serializarSolicitudProgramaReduccionGlobal(solicitud_programa_reduccion_global* solicitud){
	...
	char* serialized_items = serializar_items_prog_reduccion_global(&(solicitud->workers),solicitud->cantidad_item_programa_reduccion);
	serializarDato(serializedPackage,serialized_items,sizeof(char)*size_items,&offset);
	free(serialized_items);
...
t_worker* deserializar_items_programa_reduccion_global(char* serialized, uint32_t items_cantidad){
	int offset = 0;
...
solicitud_programa_reduccion_global* deserializarSolicitudProgramaReduccionGlobal(char* serialized){
	...
	char* serialized_items = malloc(sizeof(char)*size_items);
	deserializarDato(serialized_items,serialized,size_items,&offset);
	solicitud->workers = deserializar_items_programa_reduccion_global(serialized_items,solicitud->cantidad_item_programa_reduccion);
	free(serialized_items);
